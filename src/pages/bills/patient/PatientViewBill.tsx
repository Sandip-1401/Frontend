...
import { generateQR, getBillByAppointment, payBill } from "@/features/general/api";
import type { Bill } from "@/types/apiResponse";
import { useMutation, useQuery } from "@tanstack/react-query";
import { Calendar, Clock, Download, ReceiptText, ShoppingBag, Tag, QrCode, JapaneseYenIcon, AlertCircleIcon } from "lucide-react";
import { useParams } from "react-router-dom"
import { PDFDownloadLink } from '@react-pdf/renderer';
import MyLoader from "@/components/MyLoader";
import { BillPDF } from "../doctor/BillPDF";
import {
   Dialog,
   DialogClose,
   DialogContent,
...
const PatientViewBill = () => {
   const { appointment_id } = useParams();

   const form = useForm<PayBill>({
      mode: "onSubmit",
      defaultValues: {
         amount: 0
      }
   ...
   const mutation = useMutation({
      mutationFn: async (payload: PayBill) => {
         const res = await payBill(String(data?.bill_id), payload)
         if (!res.success) {
               ...
               <Dialog>
                  ...
                  <DialogContent className="sm:max-w-sm">
                     <DialogHeader>
                        ...
                        <DialogDescription>
                           Enter the amount to complete your payment.
                        </DialogDescription>
                     </DialogHeader>
                     <form
...
export default PatientViewBill;