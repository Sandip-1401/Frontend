...
import { getPendingForDoctor } from '@/features/general/api'
...
const PendingAppointment = () => {
  ...
  const { data, isLoading, error } = useQuery<AppointmentForDoctor[]>({
    ...
    queryFn: async () => {
      const res = await getPendingForDoctor();
      if (!res.success) {
  ...
  return (
    <div className='p-6'>
      <DynamicBreadcrumb
        ...
        />
        <div>
          <p className='text-2xl font-semibold mb-5 dark:text-cyan-400 text-gray-700'>Pending Appointments</p>
        </div>
      <DataTable
        data={formattedData}
        columns={columns}
        ...
        onView={(row) => {
          navigate(`/doctor/appointments/pending-appointment/${row.appointment_id}`)
        }}
